...
describe('Frontend', function() {
	...
	it("should redirect to dashboard view on 'Account 1' button click", function(done) {
		browser.get('http://localhost:8080').then(function() {
			setTimeout(done, 1000);
			listOfUsers.first().click();
		});
	...
	it("should load 'Account 1' as the text for my account in dashboard view", function() {
		var myAccount = element(by.css('.my-acc'));
		expect(myAccount.getText()).toBe('Account 1');
	});
	...
	it("should have Account 2 'currentPerson.called' class active, eg. call Account 2", function() {
		var activePersonsCalled = element.all(by.css('.inCallTxt'));
		expect(activePersonsCalled.count()).toBe(1);
	});

	it("should hangup to Account 2 on button click", function(done) {
